...
class backend extends \lw_object
{
    ...
    function __construct($config,$request,$repository,$pluginname,$oid, $db) 
    {
       ...
       $this->oid = $oid;
       $this->db = $db;
       $this->response = new \lwIntranetuser\Services\Response();
    }
    ...
    function backend_save()
    {
        ...
        $parameter = array();
        $parameter["show"]["logout"]        = $this->request->getInt("logout");
        $parameter["show"]["id"]            = $this->request->getInt("id");
        $parameter["show"]["username"]      = $this->request->getInt("username");
